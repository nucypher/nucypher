...
class StakerAgent(EthereumContractAgent):
    ...
    def deposit_tokens(self, amount: int, lock_periods: int, sender_address: str) -> str:
        """Send tokes to the escrow from the staker's address"""
    ...
    def sample(self, quantity: int, duration: int, additional_ursulas: float=1.7, attempts: int=5) -> List[str]:
        ...
        See full diagram here: https://github.com/nucypher/kms-whitepaper/blob/master/pdf/stakers-ruler.pdf
        """

        stakers_population = self.get_staker_population()
        if quantity > stakers_population:
            raise self.NotEnoughStakers('{} stakers are available'.format(stakers_population))
...
class PolicyAgent(EthereumContractAgent):
    ...
    def collect_policy_reward(self, collector_address: str, staker_address: str):
        ...
        policy_reward_txhash = self.contract.functions.withdraw(collector_address).transact({'from': staker_address})
