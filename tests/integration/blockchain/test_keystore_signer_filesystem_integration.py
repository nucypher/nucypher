...
def good_signer(mock_account, mock_keystore):
    ...
    signer.unlock_account(
        account=mock_account.address, password=INSECURE_DEVELOPMENT_PASSWORD
    )
...
def test_invalid_keystore(tmp_path):
    with pytest.raises(Signer.InvalidSignerURI) as e:
        Signer.from_signer_uri(
            uri=f'keystore:{tmp_path.absolute()/"nonexistent"}', testnet=True
        )
...
def test_create_signer_from_keystore_directory(mock_account, mock_keystore):
    ...
    signer = Signer.from_signer_uri(
        uri=mock_keystore_uri, testnet=True
    )  # type: KeystoreSigner
...
def test_keystore_locking(mock_account, good_signer, unknown_address, mocker):
    ...
    with pytest.raises(Signer.UnknownAccount):
        good_signer.unlock_account(
            account=unknown_address, password=INSECURE_DEVELOPMENT_PASSWORD
        )
    ...
    # Missing password
    with pytest.raises(
        Signer.AuthenticationFailed, match="No password supplied to unlock account."
    ):
    ...
    with pytest.raises(
        ...
        match="Invalid or incorrect ethereum account password.",
    ):
        good_signer.unlock_account(
            account=mock_account.address, password="imadeupthispassworditisverygood"
        )

    # Correct account and password
    successful_unlock = good_signer.unlock_account(
        account=mock_account.address, password=INSECURE_DEVELOPMENT_PASSWORD
    )
