...
def test_alice_can_learn_about_a_whole_bunch_of_ursulas(highperf_mocked_alice):
    ...
    # assert elapsed < 6  # 6 seconds is still a little long to discover 4000 out of 5000 nodes,
    # but before starting the optimization that went with this test, this operation took about 18 minutes on jMyles' laptop.
...
def test_mass_treasure_map_placement(fleet_of_highperf_mocked_ursulas,
    ...
    with patch('umbral.keys.UmbralPublicKey.__eq__', lambda *args, **kwargs: True), mock_metadata_validation:
        ...
        # TODO: This line fails sometimes because the loop goes too fast. (and too furious?)
