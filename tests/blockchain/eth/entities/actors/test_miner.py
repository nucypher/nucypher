...
def test_staker_locking_tokens(testerchain, three_agents, staker, token_economics):
    ...
    locked_tokens = staker_agent.contract.functions.getLockedTokens(staker.checksum_public_address).call()
    assert 0 == locked_tokens

    locked_tokens = staker_agent.contract.functions.getLockedTokens(staker.checksum_public_address, 1).call()
