...
def test_staker_locking_tokens(testerchain, agency, staker, token_economics):
    ...
    allowance = token_agent.contract.functions.allowance(
        staker.checksum_public_address,
        staking_agent.contract_address).call()
    assert 0 == allowance

    # Staking starts after one period
    locked_tokens = staking_agent.contract.functions.getLockedTokens(staker.checksum_public_address).call()
    assert 0 == locked_tokens

    locked_tokens = staking_agent.contract.functions.getLockedTokens(staker.checksum_public_address, 1).call()
...
def test_staker_collects_staking_reward(testerchain, staker, agency, token_economics):
    ...
    assert token_agent.get_balance(staker.checksum_public_address) == initial_balance
    ...
    final_balance = token_agent.get_balance(staker.checksum_public_address)
