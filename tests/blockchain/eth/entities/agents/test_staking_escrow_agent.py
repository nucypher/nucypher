...
def test_confirm_activity(agency, testerchain):
    ...
    staker_account, worker_account = testerchain.unassigned_accounts[2:4]

    txhash = staking_agent.set_worker(staker_address=staker_account, worker_address=worker_account)

    txhash = staking_agent.confirm_activity(worker_address=worker_account)
    receipt = testerchain.wait_for_receipt(txhash)
    assert receipt['status'] == 1, "Transaction Rejected"
...
def test_collect_staking_reward(agency, testerchain):
    ...
    # Confirm Activity
    _txhash = staking_agent.confirm_activity(worker_address=worker_account)
    testerchain.time_travel(periods=2)

    # Mint
    _txhash = staking_agent.mint(staker_address=staker_account)

    old_balance = token_agent.get_balance(address=staker_account)

    txhash = staking_agent.collect_staking_reward(staker_address=staker_account)
    receipt = testerchain.wait_for_receipt(txhash)
    assert receipt['status'] == 1, "Transaction Rejected"
