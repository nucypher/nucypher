...
def test_dispatcher(testerchain):
    ...
    # Try to deploy broken library and dispatcher for it
    contract0_bad_lib, _ = testerchain.interface.deploy_contract('ContractV0Bad')
...
def test_selfdestruct(testerchain):
    ...
    with pytest.raises((TransactionFailed, ValueError)):
        testerchain.interface.deploy_contract('Dispatcher', NULL_ADDRESS, secret_hash)
    with pytest.raises((TransactionFailed, ValueError)):
    ...
    with pytest.raises((TransactionFailed, ValueError)):
        tx = dispatcher.functions.upgrade(NULL_ADDRESS, secret, secret2_hash).transact({'from': creator})
        testerchain.wait_for_receipt(tx)
    with pytest.raises((TransactionFailed, ValueError)):
