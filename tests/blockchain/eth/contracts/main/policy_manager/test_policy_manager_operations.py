...
def test_refund(testerchain, escrow, policy_manager):
    ...
    with pytest.raises((TransactionFailed, ValueError)):
        ...
        testerchain.wait_for_receipt(tx)
    with pytest.raises((TransactionFailed, ValueError)):
        tx = policy_manager.functions.refund(policy_id, NULL_ADDRESS).transact({'from': client})
        testerchain.wait_for_receipt(tx)
    with pytest.raises((TransactionFailed, ValueError)):
    ...
    with pytest.raises((TransactionFailed, ValueError)):
        policy_manager.functions.calculateRefundValue(policy_id, NULL_ADDRESS).call({'from': client})
    ...
    with pytest.raises((TransactionFailed, ValueError)):
        ...
        testerchain.wait_for_receipt(tx)
    with pytest.raises((TransactionFailed, ValueError)):
        tx = policy_manager.functions.refund(policy_id_2, NULL_ADDRESS).transact({'from': client})
        testerchain.wait_for_receipt(tx)
    with pytest.raises((TransactionFailed, ValueError)):
        policy_manager.functions.calculateRefundValue(policy_id_2, node1).call({'from': client})
    with pytest.raises((TransactionFailed, ValueError)):
        policy_manager.functions.calculateRefundValue(policy_id_2, NULL_ADDRESS).call({'from': client})
