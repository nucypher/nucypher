...
def alice_control_test_client(federated_alice):
    web_controller = federated_alice.make_control_transport(crash_on_error=True)
    yield web_controller._web_app.test_client()


@pytest.fixture(scope='module')
def bob_control_test_client(federated_bob):
    web_controller = federated_bob.make_control_transport(crash_on_error=True)
    yield web_controller._web_app.test_client()
...
def enrico_control_test_client(capsule_side_channel):
    ...
    web_controller = enrico.make_control_transport(crash_on_error=True)
    yield web_controller._web_app.test_client()
